...
- **UA runtime and dashboard source** are fetched at a pinned commit into the ignored `vendor/understand-anything/` runtime directory.
...
vendor/understand-anything/      downloaded UA analyzer and graph dashboard
...
## Upstream UA source policy

The repository does not duplicate the full UA source tree. `scripts/fetch-ua.sh`
checks out the pinned upstream commit and then removes marketing/demo assets that
are not needed by the graph runtime. Update the pin deliberately and review the
upstream diff before upgrading.
